...
// Card to display a markdown push summary and optional metadata
// Props: { summary: string, filesChanged?: number, linesAdded?: number, linesRemoved?: number, commitsToPush?: number, targetBranch?: string, requiresConfirmation?: boolean, confirmationType?: string }
export default function PushSummaryCard({ summary, filesChanged, linesAdded, linesRemoved, commitsToPush, targetBranch, requiresConfirmation, confirmationType }) {
  return (
    <div className="w-full bg-[#1a1a1a] border border-[#333] rounded-xl p-4 text-sm text-gray-200">
      <div className="flex items-center justify-between mb-2">
        <div className="font-semibold text-white">Push Summary</div>
        <div className="text-xs text-gray-400 space-x-2">
          {typeof commitsToPush === 'number' && <span>commits: {commitsToPush}</span>}
          ...
          {targetBranch && <span>branch: {targetBranch}</span>}
